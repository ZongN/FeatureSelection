SSKM2 <- function(I,SM,MI,K,SF_id = NULL,StopOption,stop_thr){
  ...
  while(conv==0){
    ...
    # 2019/07/23 Change while loop stop condition
    if(StopOption == "FeatureSame"){
      stop_thr <- SF_id
    }
    ...
    if(iter > 1){
      ...
      }else if(StopOption == "Iteration"){
        if(iter == stop_thr){
          conv <- 1
          cat("Iteration:",iter,"\n")
        }
      }else{ # StopOption == "FeatureSame"
        if(length(sort(stop_thr) == sort(SF_id)) == length(SF_id)){
          conv <- 1
        }else if(iter == 1000){
          conv <- 1
          cat("Iteration Stop,Times:",iter,"\n")
        }
      }
    }
  }
